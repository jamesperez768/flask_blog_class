...
def index():
    ...
    #execute a query to real all posts from the posts table in the database
...
def create():
    ...
    if not title:
        ...
        conn = get_db_connection()
        conn.execute('UPDATE posts SET title = ?, content = ? WHERE id = ?', (title, content))
        conn.commit()
        conn.close()
    ...
    #create a route to delete post, delete page only with POST, post id is in url
    @app.route('/<int:id>/delete/', methode=('POST'))
    def delete(id):
        #get the post
        post = get_post(id)
        #connect
